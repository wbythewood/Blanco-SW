...
for ie=1:length(events_info)
    ...
    Mw = events_info(ie).PreferredMagnitudeValue;
	% init event structure and fill in event information 
	event.evla = evla;
	event.evlo = evlo;
    event.evdp = evdp; % wbh addition
    event.Mw = Mw;
	...
	for ista = 1:length(sta_mat_files)
		...
		ind = find(ismember({sta.traces.channel},{'LHZ','BHZ','HXZ','HHZ'}));
...
%			delete(fullfile(datacache,eventid,sta_mat_files(ista).name));
		...
		elseif parameters.component == 'BHZ'
			...
			data_starttime = LHZ.startTime;
			data_delta = 1./LHZ.sampleRate;
        elseif parameters.component == 'HXZ'
            LHZ = rm_resp(LHZ);
            stadata = LHZ.data_cor;
            datacmp = 'HXZ';
            data_starttime = LHZ.startTime;
            data_delta = 1/LHZ.sampleRate;
        elseif parameters.component == 'HHZ'
            LHZ = rm_resp(LHZ);
            stadata = LHZ.data_cor;
            datacmp = 'HHZ';
            data_starttime = LHZ.startTime;
