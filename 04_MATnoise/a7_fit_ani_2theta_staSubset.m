...
%
% wbh modification to use only selected stations
%
...
%UseStaList = 
% Load station info
[sta.nw sta.name, sta.lat, sta.lon, sta.dep] = textread(StaFn,'%s %s %f %f %f');
...
for ixsp=1:nxsp
    ...
    % cut out the stations we don't want
    if ~contains(xspinfo.sta1,sta.name)
        continue
    elseif ~contains(xspinfo.sta2,sta.name)
        continue
    end
...
for ixsp=1:nxsp
    ...
    % cut out the stations we don't want... again?
    if ~contains(xspinfo.sta1,sta.name)
        continue
    elseif ~contains(xspinfo.sta2,sta.name)
        continue
...
%%
psfile1 = [phv_fig_path,'xsp_fitani_',comp{1}(1),'_SNRtol',num2str(snr_tol),'_disttol',num2str(r_tol),'_errtol',num2str(err_tol),'_Ani-data_',SubsetStr,'.pdf'];
%print('-dpsc2',psfile);
%save2pdf(psfile1,f3,1000);
saveas(f3,psfile1);
psfile2 = [phv_fig_path,'xsp_fitani_',comp{1}(1),'_SNRtol',num2str(snr_tol),'_disttol',num2str(r_tol),'_errtol',num2str(err_tol),'_A-Phi_',SubsetStr,'.pdf'];
%print('-dpsc2',psfile);
%save2pdf(psfile2,f4,1000);
saveas(f4,psfile2);
psfile3 = [phv_fig_path,'xsp_fitani_',comp{1}(1),'_SNRtol',num2str(snr_tol),'_disttol',num2str(r_tol),'_errtol',num2str(err_tol),'_sum_ES17_',SubsetStr,'.pdf'];
%print('-dpsc2',psfile);
%save2pdf(psfile3,f5,1000);
%saveas(f5,psfile3);

psfile6 = [phv_fig_path,'xsp_fitani_',comp{1}(1),'_SNRtol',num2str(snr_tol),'_disttol',num2str(r_tol),'_errtol',num2str(err_tol),'_2theta_ES17_',SubsetStr,'.pdf'];
%print('-dpsc2',psfile);
%save2pdf(psfile6,f6,1000);
%saveas(f6,psfile6);

psfile7 = [phv_fig_path,'xsp_fitani_',comp{1}(1),'_SNRtol',num2str(snr_tol),'_disttol',num2str(r_tol),'_errtol',num2str(err_tol),'_4theta_ES17_',SubsetStr,'.pdf'];
%print('-dpsc2',psfile);
%save2pdf(psfile7,f7,1000);
saveas(f7,psfile7);

psfile8 = [phv_fig_path,'xsp_fitani_',comp{1}(1),'_SNRtol',num2str(snr_tol),'_disttol',num2str(r_tol),'_errtol',num2str(err_tol),'_Resid_',SubsetStr,'.pdf'];
%print('-dpsc2',psfile);
%save2pdf(psfile8,f8,1000);
%saveas(f8,psfile8);

psfile54 = [phv_fig_path,'xsp_fitani_',comp{1}(1),'_SNRtol',num2str(snr_tol),'_disttol',num2str(r_tol),'_errtol',num2str(err_tol),'_PhVel_',SubsetStr,'.pdf'];
saveas(f54,psfile54);
