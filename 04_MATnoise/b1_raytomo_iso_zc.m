...
%% WBH this whole loop assumes each XSP is the same length... not the case for the ZC initial step. 
% The section appears only to perform the 
...
for ixsp = 1:length(xspfiles)
    ...
    if ixsp ==1
        ...
        %waxis = temp.waxis;
        waxis = xspinfo.waxis_all;
        xspinfo.waxis = waxis; %wbh add this because it differs among XSPs
        %twloc = temp.twloc;
        xspinfo.twloc_nonan = xspinfo.twloc;
        twloc = xspinfo.twloc_all;
        xspinfo.twloc = twloc; %wbh add this because it differs among XSPs
        xspinfo.isgood = zeros(size(Tperiods));
        %xspinfo.isgood_zc = temp.isgood_zc;
        xspsum = xspinfo;
...
%     end
    ...
    for ip = 1:length(Tperiods)
        ...
        %wbh get rid of periods with no zc measurements
        if xspsum(ixsp).isgood_zc(ip) == 0
            xspsum(ixsp).isgood(ip) = 0;
        end
    end
...
%         end
        ...
        %wbh add this for different sizes
        ...
        err = smooth((abs(xspsum(ixsp).err_all)./mean(abs(xspsum(ixsp).xsp))).^2,round(length(waxis)/length(twloc)));
        if length(err) < length(waxis)
            disp('err issue... moving on')
            continue
        end
        err = err(1:length(waxis)); %try this to see if it works... 
